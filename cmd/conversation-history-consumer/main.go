...
func main() {
	log.Println("Starting Conversation History Consumer...")

	tShutdown, logger, err := telemetry.Init(context.Background(), "voice-ai-conversation-history-consumer")
	if err != nil {
	...
	llmorchestrator.StartHistoryConsumer(ctx, redisMgr, cassandraMgr)

	sigChan := make(chan os.Signal, 1)
	signal.Notify(sigChan, os.Interrupt, syscall.SIGTERM)
	<-sigChan
	log.Println("Shutting down Conversation History Consumer...")
}
